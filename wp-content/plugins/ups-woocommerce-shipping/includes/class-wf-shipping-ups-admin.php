...
class WF_Shipping_UPS_Admin
{
	...
	public function ph_editable_access_point_location( $order ){
		...
		$accesspoint_locators = get_post_meta( $order->get_id(), '_ph_accesspoint_location', true );
		...
		if( !empty( $access_point_location ) )
		{
			...
			update_post_meta( $order->get_id() , '_ph_selected_accesspoint_detail', $order_shipping_accesspoint );
		...
		if ( $this->debug ) {
			if( ! empty($cached_response) )	{
				$this->admin_diagnostic_report( "--------------------UPS Access Point Details---------------- Using Cached Response" );
			}
			$this->admin_diagnostic_report( "--------------------UPS Access Point Request----------------");
			$this->admin_diagnostic_report( $xmlRequest );
			$this->admin_diagnostic_report( "--------------------UPS Access Point Response----------------");
		...
		if(!empty($drop_locations)){
			...
			update_post_meta( $order->get_id() , '_ph_accesspoint_location', $all_locators );
		}
	...
	function ph_save_access_point_location( $post_id ){
		if(isset($_POST[ 'shipping_accesspoint' ])){
			...
			$accesspoint_locators = get_post_meta( $post_id , '_ph_accesspoint_location', true );
			...
			foreach( $accesspoint_locators as $locator_id => $locator ){
				if( $locator_id == $accesspoint_id) {
					update_post_meta( $post_id, '_shipping_accesspoint', $accesspoint_locators[$locator_id] );
					break;
				}
			}
		}	
...
<?php
	...
	*/
	...
	function wf_ups_shipment_confirmrequest( $order,$return_label=false ) {
        ...
        update_post_meta( $order->get_id(), 'ph_ship_from_address', $ship_from_address );
		...
		*/
		...
		private function wf_get_accesspoint_datas( $order_details ){
			if( WC()->version < '2.7.0' ){
				return ( isset($order_details->shipping_accesspoint) ) ? json_decode( $order_details->shipping_accesspoint ) : '';
			}else{
		...
		public function get_confirm_shipment_accesspoint_request($order_details){
			...
			$accesspoint_locators = get_post_meta( $order_details->get_id(), '_ph_accesspoint_location', true );
			
			// If $accesspoint contains locator_id only, we loop through locators to find selected locator
			if( !is_object( $accesspoint ) ) {
