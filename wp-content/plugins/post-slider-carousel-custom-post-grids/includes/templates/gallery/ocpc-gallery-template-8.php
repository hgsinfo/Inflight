...
if($ocpc_postquery->have_posts() ) {
    while($ocpc_postquery->have_posts() ) {
        ...
        if($posttype == 'attachment') {
            $ocpc_postimgurl = wp_get_attachment_image_src($post_id);
            if (!empty($ocpc_postimgurl)) {
                $ocpc_imgsrc_link = esc_url($ocpc_postimgurl[0]); 
            } else if(!empty($placeholderimage)) { 
                $ocpc_imgsrc_link = $placeholderimage;
            } else {
                $ocpc_imgsrc_link = esc_url(PSCCPG_PLUGIN_DIR .'/assets/images/placeholder.jpg');
                ?>
            <?php 
            }
        } else {
            if (has_post_thumbnail( $post_id )) {
                $ocpc_imgsrc = wp_get_attachment_image_src( get_post_thumbnail_id( $post_id ), 'medium' );
                $ocpc_imgsrc_link = $ocpc_imgsrc[0]; 
            } else if(!empty($placeholderimage)) { 
                $ocpc_imgsrc_link = esc_url($placeholderimage);
            } else {
                $ocpc_imgsrc_link = esc_url(PSCCPG_PLUGIN_DIR .'/assets/images/placeholder.jpg');  
            }
        }
    ...
    ?>        
        <div class="ocpc-topbox" style="width:<?php echo $width?>%; padding:<?php echo $ocpc_gl_space_img?>px;">
            ...
            <div class="ocpc_tmplt8_glry_main">
                <div class="ocpc_tmplt8_glry_img_div">
                    <div class="ocpc_tmplt8_glry_img">
                        <a target="_self" href="<?php echo $post_link; ?>">
                            <img alt="" src="<?php echo $ocpc_post_image_url; ?>">
                        </a>
                    </div>
                </div>
                <div class="ocpc_tmplt8_glry_cont_main">
                    <?php
                    if($ocpc_posttitle == 'yes') {
                    ?>
                        <div class="ocpc_tmplt8_glry_cont_title">
                    ...
                    ?>
                        <div class="ocpc_tmplt8_glry_cont_date">
                    ...
                    ?>
                        <div class="ocpc_tmplt8_glry_cont_cats">
                    ...
                    ?>
                        <div class="ocpc_tmplt8_glry_cont_author">
                    ...
                    ?>
                    <div class="ocpc_tmplt8_glry_cont_dec">
                        <?php echo $post_desc; ?>
                    </div>
                    <?php
                    }
